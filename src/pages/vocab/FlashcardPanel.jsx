...
export default function FlashcardPanel({ words }) {
  const [index, setIndex] = useState(0)
  const [flipped, setFlipped] = useState(false)
  const [known, setKnown] = useState([]) // cac tu da thuoc
  ...
  function toggleKnown() {
    setKnown((k) => k.includes(card.id) ? k.filter((x) => x !== card.id) : [...k, card.id])
  }
  function reset() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="w-full h-2 bg-brand-100 rounded-full mb-6 overflow-hidden">
        <div className="h-full bg-brand-500 transition-all" style={{ width: `${((index + 1) / total) * 100}%` }} />
      </div>

      {/* The flashcard - bam de lat */}
      <div
